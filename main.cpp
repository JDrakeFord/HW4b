...
string getName(){
    ...
    cin >> name;
    cout << "Welcome " << name << " to the race!" << endl;
    return name;
}
...
//
//-------------------------------------------------------------------------------------------------
int getVehicle(const int TRUCK_SPEED, const int TRUCK_CRASH, const int KART_SPEED, const int KART_CRASH, const int BIKE_SPEED, const int BIKE_CRASH){
    int choice;
    cout << "----------Cart Selection----------" << endl
         << "| Truck  |   Speed: " << TRUCK_SPEED << "   |   Crash time: " << TRUCK_CRASH << " |" << endl
         << "| Kart   |   Speed: " << KART_SPEED <<  "   |   Crash time: " << KART_CRASH  << " |" << endl
         << "| Bike   |   Speed: " << BIKE_SPEED <<  "   |   Crash time: " << BIKE_CRASH  << " |" << endl << endl;
    cout << "Select a vehicle:" << endl <<
            "1. Truck" << endl <<
            "2. Kart" << endl <<
            "3. Bike" << endl;
    do{
        ...
        if(!((choice == 1) || (choice == 2) || (choice == 3)))
            cout << "Sorry! You must chose 1, 2, or 3." << endl;
...
//-------------------------------------------------------------------------------------------------
void setVehicle(int &playerSpeed, int &playerCrash, const int choice, const int TRUCK_SPEED, const int TRUCK_CRASH,
                const int KART_SPEED, const int KART_CRASH, const int BIKE_SPEED, const int BIKE_CRASH){
    switch(choice){
        case 1:
            playerSpeed = TRUCK_SPEED;
            playerCrash = TRUCK_CRASH; break;
        case 2:
            playerSpeed = KART_SPEED;
            playerCrash = KART_CRASH; break;
        case 3:
            playerSpeed = BIKE_SPEED;
            playerCrash = BIKE_CRASH; break;
    }
...
// Title:       Function 4
// Name:        checkWinning
// Parameters:  racerProgres:  integer, pass by value, the player's position on the race track, range (0, 100)
//              bowserProgres: integer, pass by value, Bowser's position on the race track, range (0, 100)
//              luigiProgres:  integer, pass by value, Luigi's position on the race track, range (0, 100)
...
string randomBox(){
    ...
    if(box == "")
        cout << "Something is wrong with random selection! Check it!" << endl;
    return box;
}
...
{
    bool isStunned = (speed == 0);
    if(isStunned)
        cout << name << " status:  Speed = " << speed << ", " << name << " is stunned." << endl;
    else
        cout << name << " status:  Speed = " << speed << ", " << name << " is not stunned." << endl;
}
...
//-------------------------------------------------------------------------------------------------
void resetSpeed(int &racerSpeed, int &bowserSpeed, int &luigiSpeed, const int racerCarSpeed,
                const int TRUCK_SPEED, const int BIKE_SPEED){
             ...
             int & racerSpeed, int & enemyOneSpeed, int & enemyTwoSpeed, const string item)
{
    ...
    if (item == "shell")
    {
        ...
        else if (enemyTwoProgress > racerProgress)
        {
            ...
            cout << "No enemies in front to target with the shell!" << endl << endl;
    }
    ...
    if (item == "banana")
    {
        ...
        else if (enemyTwoProgress < racerProgress)
        {
            ...
            cout << "No enemies behind to target with a banana." << endl << endl;
    }
}
